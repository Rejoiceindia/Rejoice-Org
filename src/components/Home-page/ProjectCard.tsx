...
const ProjectCard : React.FC<ProjectDetailsType> = ({ projectName, description, image, techStack, links, contributors }) => {
    ...
    return (
    <div className="max-w-xl bg-white dark:bg-[#1e1c1c] dark:duration-300 border-2 hover:border-[#4F67FF] dark:border-2 border-gray-300 dark:border-gray-900 dark:hover:shadow-black hover:shadow-gray-400 rounded-lg shadow-xl overflow-hidden transform transition-all duration-500 hover:shadow-xl">
        <div className="p-6">
            ...
            <div className="flex flex-wrap gap-2 mb-4 ">
                {techStack.map((tech) => (
                    <span key={tech} className="px-3 py-1 bg-[#eeeded] dark:bg-[#0d0d0d] dark:duration-300 dark:text-gray-300 rounded-full text-sm text-gray-800 transition-all duration-300 hover:bg-gray-200 dark:hover:bg-black hover:-translate-y-1">
                        {tech}
                    </span>
                ))}
            </div>
